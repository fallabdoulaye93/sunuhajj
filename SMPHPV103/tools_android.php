...
function crediterCarte($id, $montant){
    try{
        $dbh = Connection();
        $sql = "UPDATE carte SET solde = solde + :montant WHERE id = :id";
        $stmt = $dbh->prepare($sql);
        $stmt->bindValue('montant', $montant);
        $stmt->bindValue('id', $id);
        $stmt->execute();
...
function getSoldeCarte($numcarte){
    try{
        ...
        $dbh = Connection();
        $sql = "SELECT carte.id, numero, numero_serie, client, statut, client.prenom, client.nom, client.telephone, solde FROM carte INNER JOIN client ON carte.client=client.id WHERE chip = :id AND client.etat = 1";
        $stmt = $dbh->prepare($sql);
        $stmt->bindValue('id', $numcarte);
        $stmt->execute();
        if($stmt->rowCount() == 1){
            $detailscarte = $stmt->fetchObject();
            if($numcarte === sha1($detailscarte->numero_serie).'##'.md5('NUMH@201912'.$detailscarte->numero)) {
                if($detailscarte->statut == 1){
                    return json_encode(array('errorCode' => 0, 'prenom' => $detailscarte->prenom, 'nom' => $detailscarte->nom, 'telephone' => $detailscarte->telephone, 'numero_serie' => $detailscarte->numero_serie, 'solde' => number_format($detailscarte->solde,0,'.', ' ')));
                }
                else{
                    return json_encode(array('errorCode' => 4, 'errorMessage' => 'Carte desactivée'));
                }
            }
            else{
                return json_encode(array('errorCode' => 3, 'errorMessage' => 'Données carte incohérentes'));
            }
        }
        else{
            return json_encode(array('errorCode' => 2, 'errorMessage' => 'Carte inexistante'));
        }
    }
    catch (PDOException $e){
        return json_encode(array('errorCode' => 1, 'errorMessage' => 'Erreur SQL: '.$e));
    }
}
...
function saveRechargeCarte($num_transac, $uuid, $user_id, $montant, $bus_id, $date_transac, $carte_id, $statut){
    try{
        $dbh = Connection();
        $sql = "INSERT INTO `histo_rechargement`(`num_transac`, `date_transac`, `carte_id`, `montant`, `bus_id`, `user_id`, `uuid`, `etat`) VALUES (:num_transac, :date_transac, :carte_id, :montant, :bus_id, :user_id, :uuid, :etat)";
        $stmt = $dbh->prepare($sql);
        $stmt->bindValue('num_transac', $num_transac);
        $stmt->bindValue('date_transac', $date_transac);
        $stmt->bindValue('carte_id', $carte_id);
        $stmt->bindValue('montant', $montant);
        $stmt->bindValue('bus_id', $bus_id);
        $stmt->bindValue('user_id', $user_id);
        $stmt->bindValue('uuid', $uuid);
        $stmt->bindValue('etat', $statut);
        return $stmt->execute();
    }
    catch (PDOException $e){
        return $e;
...
function getInfosCarte($numserie, $telephone){
    try{

        $dbh = Connection();
        $sql = "SELECT carte.id, numero, numero_serie, chip, client, statut, client.prenom, client.nom, client.telephone FROM carte INNER JOIN client ON carte.client=client.id WHERE carte.numero_serie = :numserie AND client.etat = 0 AND client.telephone = :telephone";
        $stmt = $dbh->prepare($sql);
        ...
        if($stmt->rowCount() == 1){
            $detailscarte = $stmt->fetchObject();
            return json_encode(array('errorCode' => 0, 'id' => $detailscarte->id, 'prenom' => $detailscarte->prenom, 'nom' => $detailscarte->nom, 'telephone' => $detailscarte->telephone, 'numero_serie' => $detailscarte->numero_serie, 'numero' => $detailscarte->numero, 'chip' => $detailscarte->chip));
        }
        else{
            return json_encode(array('errorCode' => 2, 'errorMessage' => 'Carte inexistante'));
        }

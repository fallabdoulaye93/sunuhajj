...
class MaterielsController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        $this->materielsModels = new MaterielsModel();
        ...
        $this->views->initTemplate(["header" => "header", "footer" => "footer", "sidebar" => "sidebar_materiel"]);

    }

    public function index__()
    {
        $this->views->getTemplate('materiels/badges');
    ...
    {
        //$data['tier '] = $this->materielsModels->getChauffeur();
  ...
  // var_dump($this->paramPOST);die;
        ...
        $result = $this->materielsModels->insertChauffeur(["champs" => $this->paramPOST]);
    ...
    {//var_dump($this->paramGET[2]);die();
        $data['tier '] = $this->materielsModels->getChauffeur(["condition" => ["id = " => $this->paramGET[2]]])[0];
    ...
    public function updateChauffeur()
    {
        ...
        $result = $this->materielsModels->updateChauffeur($data);
    ...
    public function updateChauffeurDetail()
    {
        ...
        unset($this->paramPOST['id']);
        $data['champs'] = $this->paramPOST;
        $result = $this->materielsModels->updateChauffeurDetail($data);
        if ($result !== false) Utils::setMessageALert(["success", $this->lang["succes_update_chauffeur"]]);
        else Utils::setMessageALert(["danger",$this->lang["echec_update_chauffeur"]]);
    ...
    public function removeChauffeur()
    {
        if (intval($this->paramGET[0]) > 0) {
            $result = $this->materielsModels->deleteChauffeur(["condition" => ["id = " => $this->paramGET[0]]]);
    ...
    { //var_dump($this->_USER);die;
        if ($this->_USER) {
            if ($this->_USER->admin == 1  || \app\core\Utils::getModel('tier ')->__authorized($this->_USER->idprofil, 'tier ', 'modifChauffeurModal') > 0) {
                ...
                $this->processing($this->materielsModels, 'getAllChauffeur', $param);
    ...
    public function detailChauffeur(){
        ...
        $data['tier '] = $this->materielsModels->getOneChauffeur(["condition" => ["e.id = " => $this->paramGET[0]]]);
    ...
    {
        if (intval($this->paramGET[0]) > 0) {
            $result = $this->materielsModels->updateChauffeur(["champs" => ["etat" => 1], "condition" => ["id = " => $this->paramGET[0]]]);
    ...
    {
        if (intval($this->paramGET[0]) > 0) {
            $result = $this->materielsModels->updateChauffeur(["champs" => ["etat" => 0], "condition" => ["id = " => $this->paramGET[0]]]);
    ...
    public function updatePhotoChauffeur()
    {
        ...
        $rst= $this->materielsModels->updatePhotoChauffeur($data);
        //var_dump($rst);
        if($rst >0)
        {
...
//        $data['tier '] = $this->materielsModels->getReceveur();
    ...
    public function ajoutReceveur__()
    {
        ...
        if (Utils::validateMail($this->paramPOST["email"])) {
            ...
            if (!$this->receveurModels->VerifEmail(['utilisateur', 'email'], $this->paramPOST["email"])) {
                ...
                $result = $this->materielsModels->insertReceveur(["champs" => $this->paramPOST]);
...
//                    "fonction" => []
//                ];
//                $this->processing($this->materielsModels, 'getAllReceveur', $param);
//
//
//            }
    ...
    {
        $data['tier '] = $this->materielsModels->getControleur();
        $this->views->setData($data);
        $this->modal();
    }
    ...
    public function listeControleurPro()
    {
        if ($this->_USER) {
            if ($this->_USER->admin == 1 || \app\core\Utils::getModel('tier ')->__authorized($this->_USER->idprofil, 'tier ', 'modifControleurmodal') > 0) {
                ...
                $this->processing($this->materielsModels, 'getAllControleur', $param);
